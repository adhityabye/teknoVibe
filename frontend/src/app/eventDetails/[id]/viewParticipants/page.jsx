    ...
    const fetchParticipants = async () => {
      try {
        ...
        if (Array.isArray(response.data.participants)) {
          ...
          console.error(
            "Participants data is not an array:",
        ...
        <div className="mx-4 md:mx-10 lg:mx-20 mt-10">
          ...
          <div className="flex justify-between w-full mb-4">
            <button
              ...
              className="text-blue-500 hover:text-blue-600 px-2 py-2 rounded"
            >
              Kembali
            </button>
            <div></div>
              ...
              {participants.map((participantData, pIndex) => (
                <tr key={pIndex} className="text-center">
                  ...
                  <td className="py-2 px-4">
                    {shortenText(participantData.namaLengkap)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.nim)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.emailUGM)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.programStudi)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.angkatan)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.idLine)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.pilihanDivisi)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.alasan)}
                  </td>
                  <td className="py-2 px-4">
                    {shortenText(participantData.uploadCV)}
                  </td>
                  <td className="py-2 px-4">
